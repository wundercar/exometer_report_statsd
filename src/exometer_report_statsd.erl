...
exometer_report(Metric, DataPoint, Extra, Value, #st{type_map = TypeMap,
                             prefix = Pfx, my_pod_name = PodIP} = St) ->
    ...
    Line = [Name, ":", value(Value), "|", type(Type)] ++ maybe_add_tags([{"my_pod_name", PodIP}]),
